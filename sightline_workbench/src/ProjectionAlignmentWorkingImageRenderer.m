classdef ProjectionAlignmentWorkingImageRenderer
    ...
    methods (Static)
        function workingImages = render(scene, request, options)
            ...
            request = ProjectionAlignmentWorkingImageRenderer.requestForSchedule( ...
                request, schedule);
            sourceCache = ProjectionAnalysisSourceCache(options.AnalysisPyramid);
            pairWorkingImages = ...
                ProjectionAlignmentWorkingImageRenderer.renderPairs( ...
                scene, request, schedule, options, sourceCache);
            ...
            workingImages.AnalysisSourceDiagnostics = sourceCache.diagnostics();
        ...
        function key = cacheKey(scene, request, options)
            ...
            key = struct(Format="ProjectionAlignmentWorkingImageCacheKey", ...
                ...
                AnalysisPyramid=options.AnalysisPyramid);
        end
    end
    ...
    methods (Static, Access = private)
        ...
        function options = mergeOptions(options)
            ...
            defaults.AnalysisPyramid = struct();
            ...
            defaults.NumericalMode = ...
                ...
                defaults.NumericalMode);
            validatedCache = ProjectionAnalysisSourceCache( ...
                defaults.AnalysisPyramid);
            defaults.AnalysisPyramid = validatedCache.Options;
            options = defaults;
        end
        ...
        function pairWorkingImages = renderPairs( ...
                scene, request, schedule, options, sourceCache)
            ...
            for pairIndex = 1:pairCount
                pairWorkingImages(pairIndex) = ...
                    ...
                    scene, request, schedule.Pairs(pairIndex), options, sourceCache);
            end
        end

        function pairWorking = renderPair( ...
                scene, request, scheduledPair, options, sourceCache)
            ...
            if options.NumericalMode == "fullSourceInverseWarp"
                [samplingGrid, layerImages, sourceDiagnostics] = ...
                    ProjectionAlignmentWorkingImageRenderer.renderAnalysisPlan( ...
                    plan, request, pairScene, sourceCache, options);
            else
            ...
            pairWorking.AnalysisSourceDiagnostics = sourceDiagnostics;
        end
        ...
        function pairWorking = emptyPairWorkingImage()
            pairWorking = struct(Pair=[0 0], PairLayerIds=strings(1, 2), ...
                ...
                AnalysisSourceDiagnostics=struct([]));
        end

        function [samplingGrid, layerImages, diagnostics] = ...
                renderAnalysisPlan(plan, request, selectedScene, sourceCache, options)
            ...
            [samplingGrid.X, samplingGrid.Y] = meshgrid(queryX, queryY);
            samplingGrid.OutputSize = outputGrid.OutputSize;
            samplingGrid.QueryWorldPoints = outputGrid.Origin + ...
                outputGrid.XAxis * samplingGrid.X(:).' + ...
                outputGrid.YAxis * samplingGrid.Y(:).';
            layerImages = struct([]);
            diagnostics = struct([]);
            for outputIndex = 1:numel(plan.Layers)
                layerPlan = plan.Layers(outputIndex);
                layer = selectedScene.layers(layerPlan.LayerIndex);
                queryPlane = PlanarProjection.worldToPlane( ...
                    samplingGrid.QueryWorldPoints, layerPlan.Plane);
                inverseTimer = tic;
                mapping = ProjectionFullSourceInverseWarp.mapCoordinates( ...
                    layerPlan.InverseWarp, queryPlane, outputGrid.OutputSize);
                inverseSeconds = toc(inverseTimer);
                requestedBand = ...
                    ProjectionAlignmentWorkingImageRenderer.analysisBandForLayer( ...
                    request, layerPlan.LayerIndex);
                if layerPlan.SourceProvider.Kind == "memory"
                    sourceBand = 1;
                else
                    sourceBand = requestedBand;
                end
                [imageData, validMask, sourceDiagnostic] = sourceCache.sample( ...
                    layer, layerPlan.SourceProvider, sourceBand, mapping, ...
                    options.Interpolation, options.InvalidFillValue);
                sourceDiagnostic.LayerIndex = layerPlan.LayerIndex;
                sourceDiagnostic.LayerId = string(layer.LayerId);
                sourceDiagnostic.InverseMappingSeconds = inverseSeconds;
                layerImage = struct( ...
                    LayerIndex=layerPlan.LayerIndex, ...
                    LayerId=string(layer.LayerId), ...
                    AnalysisBand=requestedBand, Image=imageData, ...
                    ValidMask=validMask, PlaneCoordinates=queryPlane, ...
                    SourceRows=mapping.RowCoordinates, ...
                    SourceColumns=mapping.ColumnCoordinates, ...
                    SourceObservationMask=validMask & mapping.ValidMask, ...
                    MeshSummary= ...
                    ProjectionAlignmentWorkingImageRenderer.meshSummary( ...
                    layerPlan.Mesh), SourceDiagnostics=sourceDiagnostic);
                layerImage.SourceRows(~layerImage.SourceObservationMask) = NaN;
                layerImage.SourceColumns(~layerImage.SourceObservationMask) = NaN;
                if isempty(layerImages)
                    layerImages = layerImage;
                    diagnostics = sourceDiagnostic;
                else
                    layerImages(outputIndex) = layerImage;
                    diagnostics(outputIndex) = sourceDiagnostic;
                end
            end
        end
