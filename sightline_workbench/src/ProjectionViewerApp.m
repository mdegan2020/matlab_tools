classdef ProjectionViewerApp < handle
    ...
    properties (Access = private)
        ...
        PendingInitialViewportFrame logical = false
        ...
        AlignmentNetworkConfigurationDropDown matlab.ui.control.DropDown
        ...
        AlignmentResetButton matlab.ui.control.Button
        ...
        MotionLoopPreference logical = true
        MotionFigure matlab.ui.Figure
        LayerManagerModeDropDown matlab.ui.control.DropDown
        ...
        MotionIdentityTimer
        MotionPlaybackTimer
        SelectedFootprintOutline = gobjects(0)
        ...
        AlignmentReviewConfirmationFcn = []
    end
    ...
    methods
        function app = ProjectionViewerApp( ...
            ...
            hasSavedCamera = ~isempty(viewerState) && ...
                isfield(viewerState, "Camera");
            app.PendingInitialViewportFrame = ~hasSavedCamera;
        ...
        function diagnostics = alignmentDiagnostics(app)
            ...
            try
                ...
                diagnostics.Request = struct( ...
                    ...
                    request.Options.Network.Configuration, ...
                    NetworkSensitivityPolicy= ...
                    request.Options.Network.LeaveOnePairOutPolicy, ...
                    NetworkSensitivityMaximumChildren= ...
                    request.Options.Network.MaximumLeaveOnePairOutChildren, ...
                    NetworkSensitivityTimeBudgetSeconds= ...
                    request.Options.Network.LeaveOnePairOutTimeBudgetSeconds, ...
                    ...
                    SafePreferredResidualImprovementFraction= ...
                    request.Options.SafeSolvePolicy. ...
        ...
        function setAlignmentReviewConfirmationFcn(app, confirmationFcn)
            %setAlignmentReviewConfirmationFcn Set an optional runtime decision hook.
            if ~(isempty(confirmationFcn) || isa(confirmationFcn, ...
                    "function_handle"))
                error("ProjectionViewerApp:invalidConfirmationFcn", ...
                    "Confirmation function must be empty or a function handle.");
            end
            app.AlignmentReviewConfirmationFcn = confirmationFcn;
        end
        ...
        function diagnostics = motionDiagnostics(app)
            ...
            diagnostics = struct(Active=runtime.Active, ...
                ...
                Mode=runtime.Mode, TrackCamera=runtime.TrackCamera, ...
    ...
    methods (Access = private)
        ...
        function createComponents(app)
            ...
            app.ViewVectorLabel.Layout.Column = 5;
        ...
        function closeAlignmentWorkbench(app)
            ...
            if ~isempty(app.AlignmentWorkbenchFigure) && ...
                    isvalid(app.AlignmentWorkbenchFigure)
                app.captureAlignmentControlState();
                app.AlignmentWorkbenchFigure.Visible = "off";
            end
        end

        function deleteAlignmentWorkbench(app)
            if ~isempty(app.AlignmentWorkbenchFigure) && ...
                    isvalid(app.AlignmentWorkbenchFigure)
        ...
        function createAlignmentControls(app)
            ...
            app.AlignmentResetButton = uibutton(app.AlignmentGrid, ...
                Text="Reset", Tooltip="Clear the complete alignment session", ...
                Tag="ProjectionViewerAlignmentResetButton", ...
                ButtonPushedFcn=@(~, ~) app.resetAlignmentWorkbench());
            app.AlignmentResetButton.Layout.Row = 4;
            app.AlignmentResetButton.Layout.Column = 15;
            ...
            app.AlignmentPairTable = uitable(app.AlignmentGrid, ...
                ...
                CellEditCallback=@(~, ~) app.alignmentPairTableEdited(), ...
            ...
            app.restoreAlignmentSessionPresentation();
        end
        ...
        function organizeAlignmentWorkbenchLayout(app, setupHeading, ...
            ...
            app.AlignmentSwapEyesButton.Layout.Column = 10;
            ...
            app.AlignmentResetEyesButton.Layout.Column = 11;
            ...
            setupGrid = uigridlayout(setupPanel, [2 7]);
            ...
            networkConfigurationLabel = uilabel(setupGrid, ...
                Text="Network model");
            networkConfigurationLabel.Layout.Row = 1;
            networkConfigurationLabel.Layout.Column = 7;
            app.AlignmentNetworkConfigurationDropDown = uidropdown( ...
                setupGrid, Items=cellstr(["Single pass", "Multiple passes", ...
                "Independent/custom"]), ItemsData=["singlePass", ...
                "multiplePasses", "independentViewsCustomPriors"], ...
                Value="multiplePasses", ...
                ValueChangedFcn=@(~, ~) app.alignmentSetupChanged(false), ...
                Tag="ProjectionViewerAlignmentNetworkConfigurationDropDown");
            app.AlignmentNetworkConfigurationDropDown.Layout.Row = 2;
            app.AlignmentNetworkConfigurationDropDown.Layout.Column = 7;
            ...
            app.AlignmentResetButton.Parent = workflowGrid;
            app.AlignmentResetButton.Layout.Row = 2;
            app.AlignmentResetButton.Layout.Column = 8;
            reviewHint = uilabel(workflowGrid, ...
                Text="Selection is synchronized with the match ledger");
            reviewHint.Layout.Row = 2;
            reviewHint.Layout.Column = 9;
        ...
        function resetAlignmentWorkbench(app)
            app.exitAlignmentSoloPair();
            app.clearAlignmentOverlays();
            app.clearAlignmentRoi(false);
            app.closeDenseSurfaceWindows();
            app.closeSurfaceWorkbench();
            app.AlignmentSession.reset();
            app.DenseSurfaceDiagnostics = struct();
            app.AlignmentAppliedGenerationId = "";
            app.applyAlignmentControlState(app.defaultAlignmentControlState());
            ...
            app.setAlignmentActionEnabled(false);
            app.captureAlignmentControlState();
            app.setAlignmentStatus("Alignment reset. Run Match to begin.");
        end

        function restoreAlignmentSessionPresentation(app)
            app.updateAlignmentLayerItems();
            if ~isempty(fieldnames(app.AlignmentSession.ControlState))
                app.applyAlignmentControlState( ...
                    app.AlignmentSession.ControlState);
            end
            ...
            if ~isempty(app.AlignmentRoiBounds)
                app.updateAlignmentRoiOverlay();
            end
            app.setAlignmentFilterEnabled(state.MatchRevision > 0);
            app.setAlignmentSolveEnabled(app.hasSolvableFilteredMatches());
            app.refreshAlignmentActionControls();
            app.AlignmentStatusLabel.Text = char(state.StatusText);
            app.refreshAlignmentSessionIndicators();
            app.refreshAlignmentOverlays(true);
            app.captureAlignmentControlState();
        end

        function captureAlignmentControlState(app)
            if isempty(app.AlignmentPresetDropDown) || ...
                    ~isvalid(app.AlignmentPresetDropDown)
                return
            end
            app.AlignmentSession.ControlState = struct( ...
                Reference=string(app.AlignmentReferenceDropDown.Value), ...
                Moving=string(app.AlignmentMovingDropDown.Value), ...
                Preset=string(app.AlignmentPresetDropDown.Value), ...
                Scope=string(app.AlignmentScopeDropDown.Value), ...
                Detector=string(app.AlignmentDetectorDropDown.Value), ...
                PairGraphMode=string( ...
                app.AlignmentPairGraphModeDropDown.Value), ...
                MaxPairs=double(app.AlignmentMaxPairsSpinner.Value), ...
                AllPairs=logical(app.AlignmentAllPairsCheckBox.Value), ...
                NetworkConfiguration=string( ...
                app.AlignmentNetworkConfigurationDropDown.Value), ...
                Loss=string(app.AlignmentLossDropDown.Value), ...
                Coplanarity=string(app.AlignmentCoplanarityDropDown.Value), ...
                ReferenceMotion=logical( ...
                app.AlignmentReferenceMotionCheckBox.Value), ...
                AcceptedOverlay=logical( ...
                app.AlignmentAcceptedOverlayCheckBox.Value), ...
                RejectedOverlay=logical( ...
                app.AlignmentRejectedOverlayCheckBox.Value), ...
                WorstOverlay=logical( ...
                app.AlignmentWorstOverlayCheckBox.Value), ...
                FeatureOverlay=logical( ...
                app.AlignmentFeatureOverlayCheckBox.Value));
        end

        function state = defaultAlignmentControlState(~)
            state = struct(Preset="fast", Scope="selectedPair", ...
                Detector="auto", PairGraphMode="balanced", MaxPairs=20, ...
                AllPairs=false, NetworkConfiguration="multiplePasses", ...
                Loss="projectionPlane2D", Coplanarity="none", ...
                ReferenceMotion=true, AcceptedOverlay=true, ...
                RejectedOverlay=false, WorstOverlay=false, ...
                FeatureOverlay=true);
        end

        function applyAlignmentControlState(app, state)
            controls = struct( ...
                Reference="AlignmentReferenceDropDown", ...
                Moving="AlignmentMovingDropDown", ...
                Preset="AlignmentPresetDropDown", ...
                Scope="AlignmentScopeDropDown", ...
                Detector="AlignmentDetectorDropDown", ...
                PairGraphMode="AlignmentPairGraphModeDropDown", ...
                MaxPairs="AlignmentMaxPairsSpinner", ...
                AllPairs="AlignmentAllPairsCheckBox", ...
                NetworkConfiguration= ...
                "AlignmentNetworkConfigurationDropDown", ...
                Loss="AlignmentLossDropDown", ...
                Coplanarity="AlignmentCoplanarityDropDown", ...
                ReferenceMotion="AlignmentReferenceMotionCheckBox", ...
                AcceptedOverlay="AlignmentAcceptedOverlayCheckBox", ...
                RejectedOverlay="AlignmentRejectedOverlayCheckBox", ...
                WorstOverlay="AlignmentWorstOverlayCheckBox", ...
                FeatureOverlay="AlignmentFeatureOverlayCheckBox");
            for field = reshape(string(fieldnames(controls)), 1, [])
                if ~isfield(state, field)
                    continue
                end
                control = app.(controls.(field));
                if ~isempty(control) && isvalid(control)
                    value = state.(field);
                    if isprop(control, "ItemsData") && ...
                            ~isempty(control.ItemsData) && ...
                            ~any(string(control.ItemsData) == string(value))
                        continue
                    end
                    control.Value = value;
                end
        ...
        function alignmentPairTableEdited(app)
            if isempty(app.AlignmentPairTable) || ...
                    ~isvalid(app.AlignmentPairTable) || ...
                    ~istable(app.AlignmentPairTable.Data)
                return
            end
            data = app.AlignmentPairTable.Data;
            controllerPairs = app.AlignmentPairController.Schedule.Pairs;
            controllerIds = string({controllerPairs.PairId});
            viewIds = ProjectionViewMetadata.ids(app.Scene);
            for row = 1:height(data)
                pair = [data.Moving(row) data.Reference(row)];
                identity = ProjectionViewMetadata.pairIdentity( ...
                    viewIds(pair(1)), viewIds(pair(2)));
                if ismember(identity.PairId, controllerIds)
                    app.AlignmentPairController.setPairEnabled( ...
                        identity.PairId, logical(data.Enabled(row)));
                end
            end
            app.refreshAlignmentActivePairControls();
            app.refreshSurfaceWorkbenchButton();
        ...
        function matchAlignmentWorkflow(app)
            ...
            try
                app.setAlignmentActiveStatus("workingImagePlanning", ...
                    "Planning alignment working images and pair schedule...");
                request = app.currentAlignmentRequest();
                ...
                app.completeAlignmentStage("workingImagePlanning", ...
                    sprintf("Planned %d enabled pair(s).", ...
                    size(enabledPairs, 1)));
                app.setAlignmentActiveStatus("workingImageRendering", ...
                    "Rendering bounded analysis working images...");
                ...
                app.completeAlignmentStage("workingImageRendering", ...
                    "Working-image rendering complete.");
                app.setAlignmentActiveStatus("featureMatching", ...
                    "Detecting features and matching scheduled pairs...");
                app.throwIfAlignmentCancelled();
                ...
                app.completeAlignmentStage("featureMatching", sprintf( ...
        ...
        function filterAlignmentWorkflow(app)
            ...
            try
                app.setAlignmentActiveStatus("filtering", ...
                    "Filtering raw matches and updating the evidence ledger...");
                app.throwIfAlignmentCancelled();
                ...
                app.completeAlignmentStage("filtering", sprintf( ...
        ...
        function solveAlignmentWorkflow(app)
            ...
            try
                ...
                app.setAlignmentActiveStatus("networkPreparation", ...
                    "Preparing selected evidence, network gauge, and solver inputs...");
                app.throwIfAlignmentCancelled();
                options = app.AlignmentRequest.Options;
                currentOptions = app.currentAlignmentOptions();
                ...
                app.completeAlignmentStage("solveDiagnostics", ...
                    app.alignmentResultSummary(result));
            catch ME
        ...
        function updateAlignmentSolveProgress(app, update)
            if ~isvalid(app)
                return
            end
            stage = string(update.Stage);
            if stage == "sensitivityChild"
                status = sprintf( ...
                    "Sensitivity %d/%d, iteration %d, %.1f s...", ...
                    update.DiagnosticChildIndex, ...
                    update.DiagnosticChildrenRequested, update.Iteration, ...
                    update.ElapsedSeconds);
            elseif stage == "primaryComplete"
                status = sprintf("Primary solve complete in %.1f s.", ...
                    update.ElapsedSeconds);
            else
                status = sprintf("Solving: %s, iteration %d, %.1f s...", ...
                    char(stage), update.Iteration, update.ElapsedSeconds);
            end
            app.setAlignmentActiveStatus(stage, string(status));
            drawnow limitrate nocallbacks
        end

        function previewAlignmentResult(app)
            state = app.AlignmentSession.diagnostics();
            if ~ProjectionAlignmentSafeSolvePolicy.isPreviewAllowed( ...
                    app.AlignmentResult) || state.Stage ~= "solved"
                return
            end
            previousScene = app.Scene;
        ...
        function applyAlignmentResult(app)
            state = app.AlignmentSession.diagnostics();
            decision = ProjectionAlignmentSafeSolvePolicy.decision( ...
                app.AlignmentResult);
            if ~decision.ApplyAllowed || ...
                    ~any(state.Stage == ["solved", "previewed"])
                return
            end
            if decision.ConfirmationRequired && ...
                    ~app.confirmAlignmentReviewResult(decision)
                return
            end
        ...
        function revertAlignmentResult(app)
            state = app.AlignmentSession.diagnostics();
            if ~any(state.Stage == ["previewed", "applied"])
                return
            end
            previousScene = app.Scene;
            if app.CorrectionStore.hasCurrent("applied")
        ...
        function tf = hasSurfaceWorkbenchInput(app)
            ...
            matches = app.alignmentAcceptedSolveMatches();
            if ~isfield(matches, "Matches") || isempty(matches.Matches)
                return
            end
            options = app.currentAlignmentOptions();
            minimum = options.SafeSolvePolicy.MinSolverObservationsPerPair;
            tf = any([matches.Matches.Count] >= minimum);
        end
        ...
        function context = surfaceWorkbenchContext(app)
            ...
            active = app.AlignmentPairController.currentPair();
            activePairId = "";
            if isfield(active, "PairId")
                activePairId = string(active.PairId);
            end
            entryIds = string({entries.PairId});
            if ~ismember(activePairId, entryIds)
                activePairId = entryIds(1);
            end
        ...
        function options = currentAlignmentOptions(app)
            ...
            options = ProjectionAlignmentOptions.validate(struct( ...
                ...
                Network=app.alignmentNetworkOptions(), ...
                ...
                FailOnBoundHit=true, ...
                PreferredResidualImprovementFraction=0.10), ...
        ...
        function options = alignmentNetworkOptions(app)
            mode = app.alignmentPairGraphMode();
            switch mode
                case "fast"
                    sensitivityPolicy = "notRequested";
                case "quality"
                    sensitivityPolicy = "bounded";
                otherwise
                    sensitivityPolicy = "deferred";
            end
            options = struct( ...
                Configuration=app.alignmentNetworkConfiguration(), ...
                LeaveOnePairOutPolicy=sensitivityPolicy, ...
        ...
        function configuration = alignmentNetworkConfiguration(app)
            configuration = "multiplePasses";
            if ~isempty(app.AlignmentNetworkConfigurationDropDown) && ...
                    isvalid(app.AlignmentNetworkConfigurationDropDown)
        ...
        function mask = effectiveLayerVisibilityMask(app)
            if app.MotionRuntime.Active
                ...
                layerIndices = app.currentPresentationLayerIndices();
                if ~isempty(layerIndices)
                    mask(layerIndices) = true;
                end
                return
            end
            mask = ProjectionSoloPairVisibility.effectiveMask( ...
        ...
        function setAlignmentActionEnabled(app, isEnabled)
            ...
            if isEnabled
                ...
                app.AlignmentPreviewButton.Enable = "off";
                app.AlignmentApplyButton.Enable = "off";
                app.AlignmentRevertButton.Enable = "off";
            end
        end

        function refreshAlignmentActionControls(app)
            if isempty(app.AlignmentPreviewButton) || ...
                    ~isvalid(app.AlignmentPreviewButton)
                return
            end
            decision = ProjectionAlignmentSafeSolvePolicy.decision( ...
                app.AlignmentResult);
            state = app.AlignmentSession.diagnostics();
            app.AlignmentPreviewButton.Enable = app.onOff( ...
                decision.PreviewAllowed && state.Stage == "solved");
            app.AlignmentApplyButton.Enable = app.onOff( ...
                decision.ApplyAllowed && ...
                any(state.Stage == ["solved", "previewed"]));
            app.AlignmentRevertButton.Enable = app.onOff( ...
                any(state.Stage == ["previewed", "applied"]));
        end
        ...
        function confirmed = confirmAlignmentReviewResult(app, decision)
            message = app.alignmentReviewConfirmationSummary(decision);
            if ~isempty(app.AlignmentReviewConfirmationFcn)
                confirmed = logical(app.AlignmentReviewConfirmationFcn( ...
                    decision, string(message)));
                if ~isscalar(confirmed)
                    error("ProjectionViewerApp:invalidConfirmationResult", ...
                        "Confirmation function must return a logical scalar.");
                end
                return
            end
            parent = app.AlignmentWorkbenchFigure;
            if isempty(parent) || ~isvalid(parent)
                parent = app.UIFigure;
            end
            choice = uiconfirm(parent, message, ...
                "Apply review correction?", ...
                Options=["Apply", "Cancel"], DefaultOption=2, ...
                CancelOption=2, Icon="warning");
            confirmed = string(choice) == "Apply";
        end

        function summary = alignmentReviewConfirmationSummary(app, decision)
            improvement = 100 * decision.ResidualImprovementFraction;
            result = app.AlignmentResult;
            observations = app.totalAlignmentMatchCount(result);
            trackCount = "not reported";
            if isfield(result.Diagnostics, "WorkAccounting") && ...
                    isstruct(result.Diagnostics.WorkAccounting) && ...
                    isfield(result.Diagnostics.WorkAccounting, "TrackCount")
                value = result.Diagnostics.WorkAccounting.TrackCount;
                if isnumeric(value) && isscalar(value) && isfinite(value)
                    trackCount = string(round(value));
                end
            end
            coverage = "not reported";
            if isfield(result.Diagnostics, "Selection") && ...
                    isfield(result.Diagnostics.Selection, "CoverageSummary")
                coverage = string( ...
                    result.Diagnostics.Selection.CoverageSummary);
            end
            format = "RMS %.4g -> %.4g (%+.1f%% improvement)\n" + ...
                "Active objective %.4g -> %.4g (%+.4g); " + ...
                "max incremental OPK %.4g deg\n" + ...
                "%d observations; %s tracks; spatial coverage %s\n" + ...
                "Condition %.4g; max attitude sigma %.4g deg; " + ...
                "bound hit %s\n\nReview findings: %s";
            summary = sprintf(format, ...
                decision.RmsBefore, decision.RmsAfter, improvement, ...
                decision.ActiveObjectiveBefore, decision.ActiveObjectiveAfter, ...
                decision.ActiveObjectiveChange, ...
                decision.MaximumIncrementalOpkDegrees, observations, ...
                char(trackCount), char(coverage), decision.ConditionNumber, ...
                decision.MaximumAttitudeStandardDeviationDegrees, ...
                char(app.onOff(decision.BoundHit)), ...
                char(strjoin(decision.Warnings, "; ")));
        end
        ...
        function setAlignmentStatus(app, statusText)
            app.AlignmentSession.stopStage(statusText);
            didUpdate = false;
        ...
        function setAlignmentActiveStatus(app, stage, statusText)
            app.AlignmentSession.beginStage(stage, statusText);
            if ~isempty(app.AlignmentStatusLabel) && ...
                    isvalid(app.AlignmentStatusLabel)
                app.AlignmentStatusLabel.Text = char(statusText);
                app.refreshAlignmentSessionIndicators();
        ...
        function completeAlignmentStage(app, stage, statusText)
            app.AlignmentSession.completeStage(stage, statusText);
            if ~isempty(app.AlignmentStatusLabel) && ...
                    isvalid(app.AlignmentStatusLabel)
                app.AlignmentStatusLabel.Text = char(statusText);
            end
        ...
        function refreshAlignmentSessionIndicators(app)
            ...
            if strlength(state.ActiveStage) > 0
                lines(end + 1) = "Active work: " + state.ActiveStage + ...
                    " — " + state.ActiveStatus;
            end
            if strlength(state.LastCompletedStage) > 0
                lines(end + 1) = "Last completed: " + ...
                    state.LastCompletedStage + " — " + ...
                    state.LastCompletedStatus;
            end
        ...
        function drawSelectedAlignmentMatchOverlay(app, rowData)
            ...
            if movingValid
                movingWorld = PlanarProjection.reconstruct3d( ...
                    ...
                    app.alignmentOverlayDepthOffset();
            else
                movingWorld = nan(3, 1);
            end
            if referenceValid
        ...
        function [movingPoints, referencePoints] = ...
            ...
            if any(movingMask)
                movingPoints(:, movingMask) = PlanarProjection.reconstruct3d( ...
                    ...
                    app.Scene.renderOrigin + app.alignmentOverlayDepthOffset();
            end
            if any(referenceMask)
        ...
        function offset = alignmentOverlayDepthOffset(app)
            offset = -(numel(app.Scene.layers) + 1) / 2 * ...
                app.previewLayerDepthStepMeters() * ...
        ...
        function createImageContextMenu(app)
            ...
            app.MotionImageryMenuItem = uimenu(app.ImageContextMenu, ...
                Text="Layer Manager...", Separator="on", ...
                MenuSelectedFcn=@(~, ~) app.openMotionImagery(), ...
                Tag="ProjectionViewerLayerManagerMenuItem");
        ...
        function openMotionImagery(app)
            ...
            app.MotionFigure = uifigure(Name="Layer Manager", ...
                Position=[180 180 940 560], ...
                CloseRequestFcn=@(~, ~) app.closeMotionImagery(), ...
                Tag="ProjectionViewerLayerManagerFigure");
            grid = uigridlayout(app.MotionFigure, [6 8]);
            grid.RowHeight = {"fit", "fit", "1x", "fit", "fit", "fit"};
            grid.ColumnWidth = {"fit", "1x", "fit", "fit", ...
                "fit", "fit", "fit", "fit"};
            grid.Padding = [10 10 10 10];

            uilabel(grid, Text="Layer");
            app.LayerDropDown = uidropdown(grid, ...
                Items=cellstr(app.layerDisplayNames()), ...
                ItemsData=1:numel(app.Scene.layers), ...
                Value=app.SelectedLayerIndex, ...
                ValueChangedFcn=@(~, event) app.layerSelectionChanged(event), ...
                Tag="ProjectionViewerLayerManagerLayerDropDown");
            app.LayerDropDown.Layout.Column = [2 4];
            app.MoveLayerUpButton = uibutton(grid, Text="Move up", ...
                Tag="ProjectionViewerMoveLayerUpButton", ...
                ButtonPushedFcn=@(~, ~) app.moveSelectedLayerUp());
            app.MoveLayerUpButton.Layout.Column = 5;
            app.MoveLayerDownButton = uibutton(grid, Text="Move down", ...
                Tag="ProjectionViewerMoveLayerDownButton", ...
                ButtonPushedFcn=@(~, ~) app.moveSelectedLayerDown());
            app.MoveLayerDownButton.Layout.Column = 6;
            app.VisibleCheckBox = uicheckbox(grid, Text="Visible", ...
                Value=true, ValueChangedFcn=@(~, event) app.visibleChanged(event), ...
                Tag="ProjectionViewerLayerManagerVisibleCheckBox");
            app.VisibleCheckBox.Layout.Column = [7 8];

            app.LayerManagerAllVisibleButton = uibutton(grid, ...
                Text="All visible", ...
                ButtonPushedFcn=@(~, ~) app.setAllLayersVisible(true), ...
                Tag="ProjectionViewerLayerManagerAllVisibleButton");
            app.LayerManagerAllVisibleButton.Layout.Row = 2;
            app.LayerManagerAllVisibleButton.Layout.Column = 1;
            app.LayerManagerNoneVisibleButton = uibutton(grid, ...
                Text="None visible", ...
                ButtonPushedFcn=@(~, ~) app.setAllLayersVisible(false), ...
                Tag="ProjectionViewerLayerManagerNoneVisibleButton");
            app.LayerManagerNoneVisibleButton.Layout.Row = 2;
            app.LayerManagerNoneVisibleButton.Layout.Column = 2;
            modeLabel = uilabel(grid, Text="Presentation", ...
                HorizontalAlignment="right");
            modeLabel.Layout.Row = 2;
            modeLabel.Layout.Column = 3;
            app.LayerManagerModeDropDown = uidropdown(grid, ...
                Items=["View All", "Single View", "Pair View"], ...
                ItemsData=["viewAll", "single", "pair"], ...
                Value="viewAll", ...
                ValueChangedFcn=@(~, ~) app.layerManagerModeChanged(), ...
                Tag="ProjectionViewerLayerManagerModeDropDown");
            app.LayerManagerModeDropDown.Layout.Row = 2;
            app.LayerManagerModeDropDown.Layout.Column = [4 5];
            app.LayerManagerTrackCameraCheckBox = uicheckbox(grid, ...
                Text="Track camera", Value=false, Enable="off", ...
                ValueChangedFcn=@(~, ~) app.layerManagerTrackCameraChanged(), ...
            ...
            app.MotionHoverCheckBox.Layout.Row = 4;
            ...
            app.MotionPinCheckBox.Layout.Row = 4;
            ...
            app.MotionTable = uitable(grid, ...
                ...
                ColumnWidth={65 65 "auto" "auto" "auto" "auto"}, ...
                CellEditCallback=@(~, ~) app.layerManagerTableEdited(), ...
                Tag="ProjectionViewerMotionTable");
            app.MotionTable.Layout.Row = 3;
            app.MotionTable.Layout.Column = [1 8];
            ...
            app.MotionRateSpinner.Layout.Row = 4;
            app.MotionRateSpinner.Layout.Column = 8;
            ...
            app.MotionPlayPauseButton.Layout.Row = 5;
            app.MotionPlayPauseButton.Layout.Column = 3;
            app.MotionStartExitButton = uibutton(grid, Text="View All", ...
                ButtonPushedFcn=@(~, ~) app.setLayerPresentationMode("viewAll"), ...
                Tag="ProjectionViewerMotionStartExitButton");
            app.MotionStartExitButton.Layout.Row = 5;
            app.MotionStartExitButton.Layout.Column = 4;
            app.MotionStatusLabel = uilabel(grid, Text="", ...
                WordWrap="on", Tag="ProjectionViewerMotionStatusLabel");
            app.MotionStatusLabel.Layout.Row = 6;
            app.MotionStatusLabel.Layout.Column = [1 8];
            app.motionConfigurationChanged();
            app.updateControlsFromSelectedLayer();
        end
        ...
        function data = motionConfigurationTable(app)
            ...
            visible = reshape(logical([app.Scene.layers.Visible]), [], 1);
            ...
            for index = 1:count
                ...
                if ~isempty(sequence.Frames)
                    time(index) = sequence.Frames(index).TimeText;
                else
                    time(index) = "time unavailable";
                end
            end
            data = table(include, visible, layer, viewId, passId, time, ...
                VariableNames=["Include" "Visible" "Layer" "ViewId" ...
                "Pass" "Time"]);
        end

        function layerManagerTableEdited(app)
            data = app.MotionTable.Data;
            if istable(data) && ismember("Visible", ...
                    string(data.Properties.VariableNames)) && ...
                    height(data) == numel(app.Scene.layers)
                oldOffsets = app.anaglyphPresentationOffsets();
                for layerIndex = 1:numel(app.Scene.layers)
                    app.Scene.layers(layerIndex).Visible = ...
                        logical(data.Visible(layerIndex));
                end
                app.reconcileTiledPresentationLayers( ...
                    find(app.effectiveLayerVisibilityMask()));
                app.updateAllSurfaceBlendAppearance();
                app.applyAnaglyphPresentationOffsetDelta(oldOffsets);
                app.updateControlsFromSelectedLayer();
            end
            app.motionConfigurationChanged();
        end

        function setAllLayersVisible(app, isVisible)
            oldOffsets = app.anaglyphPresentationOffsets();
            isVisible = logical(isVisible);
            for layerIndex = 1:numel(app.Scene.layers)
                app.Scene.layers(layerIndex).Visible = isVisible;
            end
            app.reconcileTiledPresentationLayers( ...
                find(app.effectiveLayerVisibilityMask()));
            app.updateAllSurfaceBlendAppearance();
        ...
        function refreshLayerManagerTable(app)
            if isempty(app.MotionTable) || ~isvalid(app.MotionTable)
                return
            end
            previousInclude = ProjectionViewMetadata.ids(app.Scene);
            if istable(app.MotionTable.Data) && ...
                    ismember("Include", ...
                    string(app.MotionTable.Data.Properties.VariableNames))
                previousInclude = string( ...
                    app.MotionTable.Data.ViewId( ...
                    logical(app.MotionTable.Data.Include)));
            end
            data = app.motionConfigurationTable();
        ...
        function layerManagerModeChanged(app)
            app.setLayerPresentationMode( ...
                string(app.LayerManagerModeDropDown.Value));
        end

        function layerManagerTrackCameraChanged(app)
            runtime = app.MotionRuntime;
            runtime.TrackCamera = logical( ...
                app.LayerManagerTrackCameraCheckBox.Value);
        ...
        function setLayerPresentationMode(app, mode)
            mode = string(mode);
            if ~isscalar(mode) || ...
                    ~any(mode == ["viewAll", "single", "pair"])
                error("ProjectionViewerApp:invalidPresentationMode", ...
                    "Presentation mode must be View All, Single View, or Pair View.");
            end
            if mode == "viewAll"
                app.exitMotionImagery();
            else
                app.enterLayerPresentationMode(mode);
            end
            if ~isempty(app.LayerManagerModeDropDown) && ...
                    isvalid(app.LayerManagerModeDropDown)
                app.LayerManagerModeDropDown.Value = char( ...
                    app.MotionRuntime.Mode);
            end
        end

        function enterLayerPresentationMode(app, mode)
            sequence = app.configuredMotionSequence();
            minimumFrames = 1 + double(mode == "pair");
            if numel(sequence.Frames) < minimumFrames
                runtime = app.MotionRuntime;
                runtime.Warning = sprintf( ...
                    "%s requires at least %s eligible layer%s.", ...
                    app.presentationModeName(mode), ...
                    app.frameCountWord(minimumFrames), ...
                    app.pluralSuffix(minimumFrames));
                app.MotionRuntime = runtime;
                app.refreshMotionStatus();
                return
            end
            wasActive = app.MotionRuntime.Active;
            runtime = app.MotionRuntime;
            if ~wasActive
                ...
                app.pauseMotionPlayback("Presentation mode changed.");
            end
            runtime.Active = true;
            runtime.Mode = mode;
            runtime.Sequence = sequence;
            runtime.Position = app.presentationPositionForSelectedLayer( ...
                sequence, mode);
        ...
        function position = presentationPositionForSelectedLayer( ...
                app, sequence, mode)
            viewId = string(app.Scene.layers(app.SelectedLayerIndex).ViewId);
            position = find(string({sequence.Frames.ViewId}) == ...
                viewId, 1, "first");
            if isempty(position)
                position = 1;
            end
            if mode == "pair"
                position = min(position, max(1, numel(sequence.Frames) - 1));
            end
        end

        function name = presentationModeName(~, mode)
            if mode == "single"
                name = "Single View";
            elseif mode == "pair"
                name = "Pair View";
        ...
        function word = frameCountWord(~, count)
            if count == 1
                word = "one";
            elseif count == 2
                word = "two";
        ...
        function motionConfigurationChanged(app)
            ...
            app.MotionStartExitButton.Enable = "on";
            app.LayerManagerModeDropDown.Enable = "on";
            if app.MotionRuntime.Active
                mode = app.MotionRuntime.Mode;
                minimumFrames = 1 + double(mode == "pair");
                if numel(sequence.Frames) < minimumFrames
                    app.exitMotionImagery();
                    runtime = app.MotionRuntime;
                    runtime.Warning = sprintf( ...
                        ...
                        app.pluralSuffix(minimumFrames));
                    app.MotionRuntime = runtime;
                    app.LayerManagerModeDropDown.Value = "viewAll";
                    app.refreshMotionStatus();
                    return
                else
                    runtime = app.MotionRuntime;
                    runtime.Sequence = sequence;
                    runtime.Position = app.presentationPositionAfterRebuild( ...
                        sequence, runtime);
                    runtime.SceneSignature = app.motionSceneSignature();
                    runtime.Warning = strjoin(sequence.Warnings, " ");
                    app.MotionRuntime = runtime;
            ...
            app.MotionPlayPauseButton.Enable = app.onOff( ...
                ...
                app.presentationPositionCount(app.MotionRuntime) > 1);
            if app.MotionRuntime.Active
                app.MotionStatusLabel.Text = char(app.motionStatusText());
            elseif ~isempty(sequence.Frames)
        ...
        function sequence = configuredMotionSequence(app)
            options = struct(LayerIndices=1:numel(app.Scene.layers));
            if isempty(app.MotionTable) || ~isvalid(app.MotionTable)
        ...
        function position = presentationPositionAfterRebuild( ...
                app, sequence, runtime)
            position = [];
            if isstruct(runtime.Sequence) && ...
                    isfield(runtime.Sequence, "Frames") && ...
                    runtime.Position >= 1 && ...
                    runtime.Position <= numel(runtime.Sequence.Frames)
                anchorViewId = string( ...
                    runtime.Sequence.Frames(runtime.Position).ViewId);
                position = find(string({sequence.Frames.ViewId}) == ...
                    anchorViewId, 1, "first");
            end
            if isempty(position)
                position = app.presentationPositionForSelectedLayer( ...
                    sequence, runtime.Mode);
            elseif runtime.Mode == "pair"
                position = min(position, numel(sequence.Frames) - 1);
            end
        end

        function text = sequenceConfigurationStatus(app, sequence)
            count = numel(sequence.Frames);
            text = sprintf("%d frame%s. %s", count, ...
                app.pluralSuffix(count), ...
        ...
        function toggleMotionImagery(app)
            if app.MotionRuntime.Active
                ...
                app.enterLayerPresentationMode("single");
            end
        end
        ...
        function exitMotionImagery(app)
            ...
            if ~isempty(app.MotionStartExitButton) && ...
                    isvalid(app.MotionStartExitButton)
                app.MotionStartExitButton.Text = "View All";
                app.MotionPlayPauseButton.Text = "Play";
                app.MotionPlayPauseButton.Enable = "off";
                app.LayerManagerModeDropDown.Value = "viewAll";
                app.LayerManagerTrackCameraCheckBox.Enable = "off";
                app.motionConfigurationChanged();
            end
            app.refreshPointerMotionCallback();
        end
        ...
        function closeMotionImagery(app)
            ...
            app.LayerDropDown = [];
            app.VisibleCheckBox = [];
            app.MoveLayerUpButton = [];
            app.MoveLayerDownButton = [];
            app.LayerManagerModeDropDown = [];
            ...
            app.MotionHoverCheckBox = [];
            app.MotionPinCheckBox = [];
            app.MotionStartExitButton = [];
            app.MotionPreviousButton = [];
            app.MotionNextButton = [];
            app.MotionPlayPauseButton = [];
            app.MotionRateSpinner = [];
            app.MotionStatusLabel = [];
        end

        function motionControlChanged(app)
            runtime = app.MotionRuntime;
            runtime.Loop = app.MotionLoopCheckBox.Value;
            app.MotionLoopPreference = logical(app.MotionLoopCheckBox.Value);
            runtime.HoverEdges = app.MotionHoverCheckBox.Value;
            runtime.IdentityPinned = app.MotionPinCheckBox.Value;
            runtime.RateFps = ProjectionMotionPlayback.rate( ...
                app.MotionRateSpinner.Value);
        ...
        function changed = stepMotion(app, delta, isPlaybackStep)
            ...
            runtime = app.MotionRuntime;
            positionCount = app.presentationPositionCount(runtime);
            requested = runtime.Position + delta;
            boundary = requested < 1 || requested > positionCount;
            if boundary && runtime.Loop
                position = mod(requested - 1, positionCount) + 1;
                changed = true;
            elseif boundary
        ...
        function success = applyMotionFrame(app)
            ...
            app.SelectedLayerIndex = layerIndices(end);
            try
                if app.MotionRuntime.Mode == "pair"
                    frames = app.MotionRuntime.Sequence.Frames( ...
                        app.MotionRuntime.Position + (0:1));
                    app.AlignmentPairController.selectViews( ...
        ...
        function count = presentationPositionCount(~, runtime)
            count = numel(runtime.Sequence.Frames);
            if runtime.Mode == "pair"
                count = max(1, count - 1);
            end
        end

        function layerIndices = currentPresentationLayerIndices(app)
            layerIndices = zeros(1, 0);
            runtime = app.MotionRuntime;
            if ~runtime.Active || runtime.Position < 1 || ...
                    runtime.Position > app.presentationPositionCount(runtime)
                return
            end
            framePositions = runtime.Position;
            if runtime.Mode == "pair"
                framePositions = runtime.Position + (0:1);
            end
            try
                layerIndices = arrayfun(@(position) ...
                    ProjectionViewMetadata.indexForId(app.Scene, ...
                    runtime.Sequence.Frames(position).ViewId), ...
                    framePositions);
            catch exception
                runtime.Warning = "Presentation frame is stale: " + ...
                    string(exception.message);
                app.MotionRuntime = runtime;
                layerIndices = zeros(1, 0);
            end
        ...
        function ready = prepareMotionLookahead(app)
            ...
            runtime = app.MotionRuntime;
            positionCount = app.presentationPositionCount(runtime);
            requested = runtime.Position + 1;
            if requested > positionCount && runtime.Loop
                requested = 1;
            end
            if requested > positionCount
                runtime.Lookahead = struct(Available=false);
                app.MotionRuntime = runtime;
                return
            end
            framePosition = requested + double(runtime.Mode == "pair");
            frame = runtime.Sequence.Frames(framePosition);
            lookahead = struct(Available=true, Ready=false, ...
                Position=requested, ViewId=string(frame.ViewId));
        ...
        function positionViewerOverlays(app)
        ...
        function updateMotionNavigationControls(app)
            runtime = app.MotionRuntime;
            count = app.presentationPositionCount(runtime);
        ...
        function showMotionIdentity(app)
            ...
            if runtime.Mode == "pair"
                second = runtime.Sequence.Frames(runtime.Position + 1);
                app.MotionIdentityLabel.Text = char(sprintf( ...
                    "%d/%d  %s + %s", runtime.Position, ...
                    app.presentationPositionCount(runtime), ...
                    frame.LayerName, second.LayerName));
            else
                app.MotionIdentityLabel.Text = char(sprintf( ...
                    "%d/%d  %s | %s | %s | %s", frame.Position, ...
                    frame.Count, frame.LayerName, frame.PassId, ...
                    frame.TimeText, frame.CorrectionStatus));
            end
        ...
        function text = motionStatusText(app)
            runtime = app.MotionRuntime;
            if ~runtime.Active
                text = "View All shows the current stored visible layers.";
                if strlength(runtime.Warning) > 0
                    text = text + " " + runtime.Warning;
                end
                return
            end
            frame = runtime.Sequence.Frames(runtime.Position);
            if runtime.Mode == "pair"
                second = runtime.Sequence.Frames(runtime.Position + 1);
                text = string(sprintf( ...
                    "Pair %d of %d: %s + %s.", runtime.Position, ...
                    app.presentationPositionCount(runtime), ...
                    frame.LayerName, second.LayerName));
                if runtime.TrackCamera
                    text = text + " Track camera is on.";
                end
            else
        ...
        function runtime = defaultMotionRuntime(app)
            runtime = struct(Active=false, Sequence=struct(), Position=0, ...
                Mode="viewAll", TrackCamera=false, ...
                Loop=app.MotionLoopPreference, HoverEdges=true, ...
        ...
        function deleteInitialViewportFrameListener(app)
            if ~isempty(app.InitialViewportFrameListener) && ...
                    isvalid(app.InitialViewportFrameListener)
                delete(app.InitialViewportFrameListener);
            end
            app.InitialViewportFrameListener = [];
        end
        ...
        function mesh = previewTileAppearanceMesh(app, mesh, layerIndex)
            ...
            mesh.Visible = app.layerSurfaceIsEffectivelyVisible( ...
        ...
        function setLayerSurfaceVisible(app, layerIndex, isVisible)
            ...
            effectiveMask = app.effectiveLayerVisibilityMask();
        ...
        function handled = handleViewportArrowKey(app, event)
            ...
            if any(app.ViewportKeyboardMode == ["single", "pair"])
                handled = true;
                if key(1) == "leftarrow"
                    app.stepMotion(-1);
        ...
        function tf = layerSurfaceIsEffectivelyVisible( ...
                app, layerIndex, alpha)
            mask = app.effectiveLayerVisibilityMask();
            tf = app.previewSurfaceIsVisible(mask(layerIndex), alpha);
        end
        ...
        function updateControlsFromSelectedLayer(app)
            layer = app.Scene.layers(app.SelectedLayerIndex);
            app.updateLayerDropDownItems();
            if ~isempty(app.LayerDropDown) && isvalid(app.LayerDropDown)
        ...
        function setSelectedLayerVisible(app, isVisible)
            ...
            if layer.Visible == isVisible
                if ~isempty(app.VisibleCheckBox) && ...
                        isvalid(app.VisibleCheckBox)
                    app.VisibleCheckBox.Value = layer.Visible;
                end
                return
            end
        ...
        function refreshSelectedFootprintOutline(app)
            ...
            if ~visible
                if ~isempty(app.SelectedFootprintOutline) && ...
                        isgraphics(app.SelectedFootprintOutline)
                    app.SelectedFootprintOutline.Visible = "off";
                end
                return
            end
            points = app.selectedLayerOutlineRenderPoints();
            if size(points, 2) < 3
                if ~isempty(app.SelectedFootprintOutline) && ...
                        isgraphics(app.SelectedFootprintOutline)
                    app.SelectedFootprintOutline.Visible = "off";
            ...
            plane = app.Scene.layers( ...
                app.SelectedLayerIndex).CurrentProjectionPlane;
            worldPoints = points + app.Scene.renderOrigin;
            planeCoordinates = plane.basis.' * ...
                (worldPoints - plane.P0);
            finite = all(isfinite(planeCoordinates), 1);
            planeCoordinates = unique( ...
                planeCoordinates(:, finite).', "rows", "stable").';
            if size(planeCoordinates, 2) < 3
                if ~isempty(app.SelectedFootprintOutline) && ...
                        isgraphics(app.SelectedFootprintOutline)
                    app.SelectedFootprintOutline.Visible = "off";
                end
            ...
            hull = convhull(planeCoordinates(1, :).', ...
                planeCoordinates(2, :).');
            outline = PlanarProjection.reconstruct3d( ...
                planeCoordinates(:, hull), plane) - app.Scene.renderOrigin;
            topOffset = -(numel(app.Scene.layers) + 1) * ...
                app.previewLayerDepthStepMeters() * ...
                app.frameCameraViewDirection();
            outline = outline + topOffset;
            if isempty(app.SelectedFootprintOutline) || ...
                    ~isgraphics(app.SelectedFootprintOutline)
                app.SelectedFootprintOutline = line(app.Axes, ...
                    outline(1, :), outline(2, :), outline(3, :), ...
                    Color=[1 1 0], LineWidth=2.5, Clipping="on", ...
                    HitTest="off", PickableParts="none", ...
                    Tag="ProjectionViewerSelectedFootprintOutline");
            else
                app.SelectedFootprintOutline.XData = outline(1, :);
                app.SelectedFootprintOutline.YData = outline(2, :);
                app.SelectedFootprintOutline.ZData = outline(3, :);
                app.SelectedFootprintOutline.Visible = "on";
            end
            uistack(app.SelectedFootprintOutline, "top");
            app.raiseCrosshairOverlay();
        end

        function points = selectedLayerOutlineRenderPoints(app)
            layerIndex = app.SelectedLayerIndex;
            if app.usesTiledPreview(layerIndex)
                geometry = app.previewGeometryCacheForLayer(layerIndex);
                points = geometry.LayerRenderPoints;
                return
            end
            ...
            for index = 1:numel(handles)
                ...
                points = [points, [handle.XData(:).'; ...
                    handle.YData(:).'; handle.ZData(:).']]; %#ok<AGROW>
            end
        end
        ...
        function runtime = viewerPerformanceRuntimeState(app)
            ...
            surfaceVisibilityByLayer = false(1, layerCount);
            ...
            for layerIndex = 1:layerCount
                ...
                for surfaceIndex = 1:numel(surfaceHandles)
                    ...
                    surfaceVisibilityByLayer(layerIndex) = true;
            ...
            runtime.SurfaceVisibilityByLayer = surfaceVisibilityByLayer;
            ...
            runtime.AlignmentControlsCreated = false;
        ...
        function text = helpText(~)
            text = [
                ...
                "Single/Pair View: Left/Right steps frames or overlapping pairs"
                "Single/Pair View: Space toggles 0.5-10 fps Play/Pause"
                "Escape returns to View All and restores the prior camera"
