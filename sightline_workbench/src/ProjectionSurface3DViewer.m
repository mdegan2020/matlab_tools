classdef ProjectionSurface3DViewer < handle
    ...
    methods (Access = private)
        function createComponents(app)
            ...
            controls = uigridlayout(app.GridLayout, [1 10], ...
                ColumnWidth={"fit", "1x", "fit", "1x", "fit", "1x", ...
                "fit", 85, "fit", "fit"}, Padding=[0 0 0 0]);
            ...
            xlabel(app.Axes, "World X");
            ylabel(app.Axes, "World Y");
            zlabel(app.Axes, "World Z");
            grid(app.Axes, "on");
            view(app.Axes, 3);
        ...
        function refresh(app)
            ...
            axis(app.Axes, "tight");
            view(app.Axes, 3);
            title(app.Axes, app.CurrentPayload.Label, Interpreter="none");
            app.StatusLabel.Text = sprintf( ...
                "%d/%d displayed; %s; complete product retained", ...
                app.CurrentPayload.DisplayPointCount, ...
                app.CurrentPayload.FullPointCount, ...
                app.CurrentPayload.Representation);
        end
        ...
        function handles = renderPayload(app, payload, comparison)
            ...
            if comparison
                handles = scatter3(app.Axes, payload.PointsWorld(1, :), ...
                    payload.PointsWorld(2, :), payload.PointsWorld(3, :), ...
                    22, [0.35 0.35 0.35], "x", ...
                    Tag="ProjectionSurface3DComparisonObject", ...
                    HitTest="off");
                return
            end
            callback = @(~, event) app.objectSelected(event);
            switch payload.Representation
                ...
                case "mesh"
                    handles = patch(app.Axes, ...
                        ...
                        ButtonDownFcn=callback);
                otherwise
                    cdata = nan(size(payload.Grid.Z));
                    cdata(payload.Grid.ValidMask) = payload.ColorValues;
                    handles = surf(app.Axes, payload.Grid.X, payload.Grid.Y, ...
                        payload.Grid.Z, cdata, EdgeColor="none", ...
                        Tag="ProjectionSurface3DGridObject", ...
                        ButtonDownFcn=callback);
            end
        ...
        function objectSelected(app, event)
            ...
            [~, index] = min(vecnorm(app.CurrentPayload.PointsWorld - point, 2, 1));
