classdef ProjectionAlignmentOpkSolver
    ...
    methods (Static)
        function result = solve(scene, matchResult, options, runtimeControl)
            ...
            monitor.setStage(runtimeControl.Stage, struct());
            ...
            optimizerStart = x0;
            if ~isempty(runtimeControl.InitialCorrections)
                initialScene = ProjectionAlignmentOpkSolver.setCorrections( ...
                    scene, runtimeControl.InitialCorrections);
                optimizerStart = ProjectionAlignmentOpkSolver. ...
                    parametersForScene(parameterModel, initialScene);
                optimizerStart = min(parameterModel.UpperBounds, ...
                    max(parameterModel.LowerBounds, optimizerStart));
            end
            ...
            compileTimer = tic;
            compiledEvidence = ProjectionAlignmentCompiledEvidence.compile( ...
                scene, matchResult, parameterModel, monitor);
            monitor.recordTiming("CompiledEvidence", toc(compileTimer));
            options = ProjectionAlignmentOpkSolver.resolveRobustScale( ...
                options, x0, scene, matchResult, parameterModel, commonPlane, ...
                monitor, compiledEvidence);
            ...
            beforeResiduals = ProjectionAlignmentOpkSolver.dataResiduals( ...
                ...
                monitor, compiledEvidence);
            initialRobustResiduals = ...
                ProjectionAlignmentOpkSolver.robustResiduals( ...
                beforeResiduals, options.Regularization);
            initialPriorResiduals = ...
                ProjectionAlignmentParameterModel.regularizationResiduals( ...
                parameterModel, x0);
            initialObjective = sum(initialRobustResiduals .^ 2) + ...
                sum(initialPriorResiduals .^ 2);

            solverOptions = optimoptions("lsqnonlin", Display="off", ...
                MaxIterations=100, FunctionTolerance=1e-10, StepTolerance=1e-10);
            useSuppliedJacobian = ...
                ProjectionAlignmentOpkSolver.supportsSuppliedJacobian( ...
                options, parameterModel, compiledEvidence);
            solverOptions.SpecifyObjectiveGradient = useSuppliedJacobian;
            solverOptions.OutputFcn = @(x, optimValues, state) ...
                monitor.optimizerOutput(x, optimValues, state);
            if useSuppliedJacobian
                [~, ~] = residualFcn(x0);
            end
            monitor.setStage(runtimeControl.Stage, struct( ...
                ObservationCount=sum([matchResult.Matches.Count]), ...
                ActiveParameterCount=nnz(lowerBounds ~= upperBounds)));
            optimizerTimer = tic;
            [xSolved, residual, ~, exitFlag, output] = lsqnonlin( ...
                residualFcn, optimizerStart, lowerBounds, upperBounds, solverOptions);
            monitor.recordTiming("Optimizer", toc(optimizerTimer));
            monitor.recordOptimizerResult(output);
            ...
            afterResiduals = ProjectionAlignmentOpkSolver.dataResiduals( ...
                xSolved, scene, matchResult, parameterModel, commonPlane, ...
                options, monitor, compiledEvidence);
            if useSuppliedJacobian
                [~, ~] = residualFcn(xSolved);
            end
            perPairResiduals = ProjectionAlignmentOpkSolver.residualPairs( ...
                beforeResiduals, afterResiduals, matchResult, options);
            comparisonTimer = tic;
            comparisonDiagnostics = ProjectionAlignmentOpkSolver.comparisonDiagnostics( ...
                x0, xSolved, scene, matchResult, parameterModel, commonPlane, ...
                options, monitor, compiledEvidence, beforeResiduals, ...
                afterResiduals);
            monitor.recordTiming("Comparison", toc(comparisonTimer));
            observabilityTimer = tic;
            observabilityDiagnostics = ...
                ProjectionAlignmentOpkSolver.observabilityDiagnostics( ...
                x0, xSolved, scene, matchResult, parameterModel, ...
                commonPlane, options, monitor, compiledEvidence);
            monitor.recordTiming("Observability", toc(observabilityTimer));
            ...
            result = ProjectionAlignmentOpkSolver.resultStruct( ...
                ...
                initialObjective, exitFlag, output, comparisonDiagnostics, ...
                observabilityDiagnostics, options, toc(timer));
            result.Diagnostics.EvidenceEvaluation = struct( ...
                Mode=compiledEvidence.Mode, Enabled=compiledEvidence.Enabled, ...
                FallbackReason=compiledEvidence.FallbackReason, ...
                GeometryFingerprints= ...
                compiledEvidence.GeometryFingerprints);
            if useSuppliedJacobian
                jacobianMethod = "compiledStableSemiAnalytic";
                jacobianFallback = "";
            else
                jacobianMethod = "optimizerFiniteDifference";
                jacobianFallback = compiledEvidence.FallbackReason;
            end
            [linearizationAvailable, ~, solutionJacobian] = ...
                monitor.evaluation(xSolved);
            if linearizationAvailable
                jacobianSize = size(solutionJacobian);
                nonzeroCount = nnz(solutionJacobian);
                density = nonzeroCount / max(numel(solutionJacobian), 1);
            else
                jacobianSize = [0 0];
                nonzeroCount = 0;
                density = 0;
            end
            result.Diagnostics.Jacobian = struct( ...
                Requested=options.Network.JacobianMethod, ...
                Method=jacobianMethod, FallbackReason=jacobianFallback, ...
                SuppliedToOptimizer=useSuppliedJacobian, ...
                LinearizationAvailable=linearizationAvailable, ...
                Size=jacobianSize, NonzeroCount=nonzeroCount, ...
                Density=density, Storage="dense", Provider="MATLAB double");
            if options.Execution.Mode == "threads"
                parallelReason = "thread batching disabled: no measured " + ...
                    "benefit at the 5-view/2000-observation proxy";
            else
                parallelReason = "serial execution requested";
            end
            result.Diagnostics.ParallelEvaluation = struct( ...
                RequestedMode=options.Execution.Mode, UsedMode="serial", ...
                ThreadPoolUsed=false, Reason=parallelReason);
            work = monitor.snapshot();
            work.ViewCount = numel(layerIndices);
            work.PairCount = numel(matchResult.Matches);
            work.ObservationCount = sum([matchResult.Matches.Count]);
            work.ActiveParameterCount = nnz(lowerBounds ~= upperBounds);
    ...
    methods (Static, Access = private)
        function x = parametersForScene(parameterModel, scene)
            x = parameterModel.X0;
            effectiveOpk = zeros(parameterModel.LayerCount, 3);
            ...
            x(1:(3 * parameterModel.LayerCount)) = ...
                ProjectionAlignmentParameterModel. ...
                attitudeParametersForEffective(parameterModel, effectiveOpk);
        end
        ...
        function runtimeControl = validateRuntimeControl(runtimeControl)
            ...
            allowedFields = ["CancellationFcn", "ProgressFcn", ...
                "ProgressIntervalSeconds", "ProgressMonitor", ...
                "InitialCorrections", "Stage"];
            ...
            if ~isfield(runtimeControl, "ProgressFcn")
                runtimeControl.ProgressFcn = [];
            end
            if ~isempty(runtimeControl.ProgressFcn) && ...
                    ~isa(runtimeControl.ProgressFcn, "function_handle")
                error("ProjectionAlignmentOpkSolver:invalidRuntimeControl", ...
                    "ProgressFcn must be empty or a function handle.");
            end
            if ~isfield(runtimeControl, "ProgressIntervalSeconds")
                runtimeControl.ProgressIntervalSeconds = 0.10;
            end
            interval = runtimeControl.ProgressIntervalSeconds;
            if ~isnumeric(interval) || ~isscalar(interval) || ...
                    ~isfinite(interval) || interval < 0
                error("ProjectionAlignmentOpkSolver:invalidRuntimeControl", ...
                    "ProgressIntervalSeconds must be a nonnegative finite scalar.");
            end
            runtimeControl.ProgressIntervalSeconds = double(interval);
            if ~isfield(runtimeControl, "InitialCorrections")
                runtimeControl.InitialCorrections = [];
            end
            if ~isfield(runtimeControl, "Stage")
                runtimeControl.Stage = "primaryOptimization";
            end
            runtimeControl.Stage = string(runtimeControl.Stage);
            if ~isscalar(runtimeControl.Stage) || ...
                    strlength(runtimeControl.Stage) == 0
                error("ProjectionAlignmentOpkSolver:invalidRuntimeControl", ...
                    "Stage must be a nonempty scalar string.");
            end
            ...
            elseif ~isa(runtimeControl.ProgressMonitor, ...
                    "ProjectionAlignmentSolveMonitor")
                error("ProjectionAlignmentOpkSolver:invalidRuntimeControl", ...
        ...
        function [residuals, jacobian] = residualVector(x, scene, matchResult, ...
                parameterModel, commonPlane, options, monitor, compiledEvidence)
            if nargout > 1
                [dataResiduals, dataJacobian] = ...
                    ProjectionAlignmentOpkSolver.dataResidualsAndJacobian( ...
                    x, scene, matchResult, parameterModel, commonPlane, ...
                    options, monitor, compiledEvidence);
            else
                dataResiduals = ProjectionAlignmentOpkSolver.dataResiduals( ...
                    x, scene, matchResult, parameterModel, commonPlane, ...
                    options, monitor, compiledEvidence);
                dataJacobian = zeros(numel(dataResiduals), numel(x));
            end
            [dataResiduals, robustDerivative] = ...
                ProjectionAlignmentOpkSolver.robustResidualsAndDerivative( ...
                dataResiduals, options.Regularization);
            dataJacobian = dataJacobian .* robustDerivative(:);
            ...
            if nargout > 1
                priorJacobian = ...
                    ProjectionAlignmentOpkSolver.regularizationJacobian( ...
                    parameterModel, x);
                jacobian = [dataJacobian; priorJacobian];
                monitor.storeEvaluation(x, dataResiduals, dataJacobian);
            end
        end
        ...
        function residuals = dataResiduals(x, scene, matchResult, parameterModel, ...
            ...
            if nargin < 8
                compiledEvidence = struct(Enabled=false);
            end
            if ~isempty(monitor)
                monitor.increment("DataResidualEvaluations", 1);
                if ~compiledEvidence.Enabled
                    monitor.increment("ObservationSamplingCalls", ...
                        2 * numel(matchResult.Matches));
                    monitor.increment("SampledObservationCount", ...
                        2 * sum([matchResult.Matches.Count]));
                end
            end
            ...
            if ~isempty(monitor)
                monitor.recordTiming("ResidualEvaluation", toc(residualTimer));
            end
        end

        function residuals = dataResidualsByPair(x, scene, matchResult, ...
                parameterModel, commonPlane, options, compiledEvidence)
            if nargin < 7
                compiledEvidence = struct(Enabled=false);
            end
            if compiledEvidence.Enabled
                residuals = ProjectionAlignmentCompiledEvidence.evaluate( ...
                    compiledEvidence, scene, matchResult, parameterModel, x, ...
                    commonPlane, options.LossMode);
                return
            end
        ...
        function tf = supportsSuppliedJacobian( ...
                options, parameterModel, compiledEvidence)
            tf = options.Network.JacobianMethod == "semiAnalytic" && ...
                compiledEvidence.Enabled && ...
                options.LossMode == "epipolarCoplanarity" && ...
                parameterModel.SharedScaleIndex == 0;
        end

        function [residuals, jacobian] = dataResidualsAndJacobian( ...
                x, scene, matchResult, parameterModel, commonPlane, ...
                options, monitor, compiledEvidence)
            residuals = ProjectionAlignmentOpkSolver.dataResiduals( ...
                x, scene, matchResult, parameterModel, commonPlane, ...
                options, monitor, compiledEvidence);
            jacobian = zeros(numel(residuals), numel(x));
            attitudeCount = 3 * parameterModel.LayerCount;
            steps = ProjectionAlignmentOpkSolver.observabilitySteps( ...
                parameterModel);
            for parameterIndex = 1:attitudeCount
                xPlus = x;
                xMinus = x;
                xPlus(parameterIndex) = xPlus(parameterIndex) + ...
                    steps(parameterIndex);
                xMinus(parameterIndex) = xMinus(parameterIndex) - ...
                    steps(parameterIndex);
                plusByPair = ProjectionAlignmentCompiledEvidence.evaluate( ...
                    compiledEvidence, scene, matchResult, parameterModel, ...
                    xPlus, commonPlane, options.LossMode);
                minusByPair = ProjectionAlignmentCompiledEvidence.evaluate( ...
                    compiledEvidence, scene, matchResult, parameterModel, ...
                    xMinus, commonPlane, options.LossMode);
                plus = ProjectionAlignmentOpkSolver.flattenResidualCells( ...
                    plusByPair);
                minus = ProjectionAlignmentOpkSolver.flattenResidualCells( ...
                    minusByPair);
                jacobian(:, parameterIndex) = ...
                    (plus - minus) / (2 * steps(parameterIndex));
            end
            if ~isempty(monitor)
                monitor.increment("JacobianEvaluations", 1);
            end
        end

        function residuals = flattenResidualCells(pairResiduals)
            values = cellfun(@(value) value(:), pairResiduals, ...
                UniformOutput=false);
            residuals = vertcat(values{:});
        end
        ...
        function [residuals, derivative] = ...
                robustResidualsAndDerivative(residuals, regularization)
            original = residuals;
            residuals = ProjectionAlignmentOpkSolver.robustResiduals( ...
                residuals, regularization);
            derivative = ones(size(original));
            if regularization.RobustLoss == "none"
                return
            end
            scale = regularization.RobustScale;
            ...
            if regularization.RobustLoss == "huber"
                derivative(large) = scale ./ max(abs(residuals(large)), eps);
            elseif regularization.RobustLoss == "bisquare"
                derivative(large) = 0;
            elseif regularization.RobustLoss == "cauchy"
                nonzero = magnitude > eps;
                logarithm = log1p((magnitude(nonzero) ./ scale).^2);
                derivative(nonzero) = scale .* magnitude(nonzero) ./ ...
                    ((scale^2 + magnitude(nonzero).^2) .* ...
                    sqrt(logarithm));
            end
        end

        function jacobian = regularizationJacobian(parameterModel, x)
            base = ProjectionAlignmentParameterModel. ...
                regularizationResiduals(parameterModel, x);
            jacobian = zeros(numel(base), numel(x));
            steps = ProjectionAlignmentOpkSolver.observabilitySteps( ...
                parameterModel);
            for parameterIndex = 1:numel(x)
                xPlus = x;
                xMinus = x;
                xPlus(parameterIndex) = xPlus(parameterIndex) + ...
                    steps(parameterIndex);
                xMinus(parameterIndex) = xMinus(parameterIndex) - ...
                    steps(parameterIndex);
                plus = ProjectionAlignmentParameterModel. ...
                    regularizationResiduals(parameterModel, xPlus);
                minus = ProjectionAlignmentParameterModel. ...
                    regularizationResiduals(parameterModel, xMinus);
                jacobian(:, parameterIndex) = ...
                    (plus(:) - minus(:)) / (2 * steps(parameterIndex));
            end
        end
        ...
        function options = resolveRobustScale(options, x0, scene, ...
                matchResult, parameterModel, commonPlane, monitor, ...
                compiledEvidence)
        ...
        function result = resultStruct(matchResult, parameterModel, xSolved, ...
                ...
                solverResiduals, initialObjective, exitFlag, output, ...
                comparisonDiagnostics, observabilityDiagnostics, options, ...
                totalSeconds)
            ...
            finalObjective = sum(solverResiduals .^ 2);
            result.Diagnostics.ActiveObjective = struct( ...
                Before=initialObjective, After=finalObjective, ...
                Change=finalObjective - initialObjective);
            ...
            robustDataResiduals = ProjectionAlignmentOpkSolver.robustResiduals( ...
                afterResiduals, options.Regularization);
            priorResiduals = ...
                ProjectionAlignmentParameterModel.regularizationResiduals( ...
                parameterModel, xSolved);
            dataObjective = sum(robustDataResiduals.^2);
            priorObjective = sum(priorResiduals.^2);
            result.Diagnostics.PriorContribution = struct( ...
                DataObjective=dataObjective, PriorObjective=priorObjective, ...
                Fraction=priorObjective / max(dataObjective + priorObjective, eps), ...
                PriorDominated=priorObjective > dataObjective);
        ...
        function pairs = residualPairs( ...
                beforeResiduals, afterResiduals, matchResult, options)
            pairs = struct("Pair", {}, "Before", {}, "After", {}, "Count", {});
            first = 1;
            componentCount = ProjectionAlignmentOpkSolver. ...
                lossComponentCount(options.LossMode);
            for k = 1:numel(matchResult.Matches)
                count = matchResult.Matches(k).Count * componentCount;
                ...
                pairs(k).Before = ProjectionAlignmentOpkSolver.residualNorms( ...
                    beforeResiduals(indices), componentCount);
                pairs(k).After = ProjectionAlignmentOpkSolver.residualNorms( ...
                    afterResiduals(indices), componentCount);
        ...
        function diagnostics = comparisonDiagnostics(xBefore, xAfter, scene, ...
                matchResult, parameterModel, commonPlane, options, monitor, ...
                compiledEvidence, activeBefore, activeAfter)
            if nargin < 8
                monitor = [];
            end
            if nargin < 9
            ...
            for k = 1:numel(modes)
                ...
                if modes(k) == options.LossMode && ~isempty(activeBefore)
                    before = activeBefore;
                    after = activeAfter;
                    if ~isempty(monitor)
                    ...
                    before = ProjectionAlignmentOpkSolver.dataResiduals( ...
                        xBefore, scene, matchResult, parameterModel, ...
                        commonPlane, metricOptions, monitor, compiledEvidence);
                    after = ProjectionAlignmentOpkSolver.dataResiduals( ...
                        xAfter, scene, matchResult, parameterModel, ...
                        commonPlane, metricOptions, monitor, compiledEvidence);
                    if ~isempty(monitor)
                        monitor.increment("ComparisonFamilyEvaluations", 2);
                    end
                end
                componentCount = ...
            ...
            diagnostics.EpipolarCoplanarity.PerPair = ...
                ...
                commonPlane, options, compiledEvidence);
        end
        ...
        function diagnostics = coplanarityDiagnosticsByPair( ...
            ...
            if compiledEvidence.Enabled
                [~, beforeEvaluations] = ...
                    ProjectionAlignmentCompiledEvidence.evaluate( ...
                    compiledEvidence, scene, matchResult, parameterModel, ...
                    xBefore, commonPlane, "epipolarCoplanarity");
                [~, afterEvaluations] = ...
                    ProjectionAlignmentCompiledEvidence.evaluate( ...
                    compiledEvidence, scene, matchResult, parameterModel, ...
                    xAfter, commonPlane, "epipolarCoplanarity");
            else
            ...
            for k = 1:numel(matchResult.Matches)
                ...
                if compiledEvidence.Enabled
                    before = beforeEvaluations{k};
                    after = afterEvaluations{k};
                else
        ...
        function diagnostics = observabilityDiagnostics(xStart, xSolved, ...
            ...
            diagnostics = struct();
            if ProjectionAlignmentOpkSolver.supportsSuppliedJacobian( ...
                    options, parameterModel, compiledEvidence)
                diagnostics.Method = ...
                    "reusedCompiledSemiAnalyticRobustDataJacobian";
            else
                diagnostics.Method = ...
        ...
        function diagnostics = observabilityAt(x, scene, matchResult, ...
            ...
            if ~isempty(monitor)
                monitor.increment("ObservabilityEvaluations", 1);
            end
            [transform, labels, priorSupported, fixed] = ...
                ProjectionAlignmentParameterModel.modeTransform(parameterModel);
            found = false;
            if ~isempty(monitor)
                [found, ~, jacobian] = monitor.evaluation(x);
            end
            if ~found
                if ProjectionAlignmentOpkSolver.supportsSuppliedJacobian( ...
                        options, parameterModel, compiledEvidence)
                    [raw, jacobian] = ProjectionAlignmentOpkSolver. ...
                        dataResidualsAndJacobian(x, scene, matchResult, ...
                        parameterModel, commonPlane, options, monitor, ...
                        compiledEvidence);
                    [~, robustDerivative] = ProjectionAlignmentOpkSolver. ...
                        robustResidualsAndDerivative( ...
                        raw, options.Regularization);
                    jacobian = jacobian .* robustDerivative(:);
                else
                    ...
                    for k = 1:numel(x)
                        ...
                        plus = ProjectionAlignmentOpkSolver.dataResiduals( ...
                            xPlus, scene, matchResult, parameterModel, ...
                            commonPlane, options, monitor, compiledEvidence);
                        minus = ProjectionAlignmentOpkSolver.dataResiduals( ...
                            xMinus, scene, matchResult, parameterModel, ...
                            commonPlane, options, monitor, compiledEvidence);
                        ...
                        jacobian(:, k) = ...
                            (plus(:) - minus(:)) / (2 * steps(k));
                    end
                end
            end
            priorJacobian = ProjectionAlignmentOpkSolver. ...
                regularizationJacobian(parameterModel, x);
            ...
            if ~isempty(monitor)
                monitor.increment("NormalProductBuilds", 1);
            end
            ...
            effectiveAttitudeCovariance = ...
                ProjectionAlignmentParameterModel. ...
                effectiveAttitudeCovariance( ...
                parameterModel, parameterCovariance);
            diagnostics = struct(JacobianSize=size(modeJacobian), ...
                ...
                EffectiveAttitudeCovariance=effectiveAttitudeCovariance, ...
