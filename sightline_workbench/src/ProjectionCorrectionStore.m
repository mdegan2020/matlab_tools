classdef ProjectionCorrectionStore < handle
    ...
    methods
        ...
        function [scene, applied, effects] = apply(store, generationId)
            ...
            if isfield(correctionSet.Diagnostics, "ApplicationSupported")
                support = correctionSet.Diagnostics.ApplicationSupported;
                if ~(islogical(support) || isnumeric(support)) || ...
                        ~isscalar(support) || ...
                        ~isfinite(double(support)) || ~logical(support)
                    error("ProjectionCorrectionStore:unsupportedApplication", ...
                        "This accepted CorrectionSet is review-only and cannot be applied.");
                end
            end
