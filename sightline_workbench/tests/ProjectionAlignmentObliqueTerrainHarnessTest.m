classdef ProjectionAlignmentObliqueTerrainHarnessTest < matlab.unittest.TestCase
    ...
    methods (Test)
        ...
        function testTruthfulFiltersPreserveReliefRichCorrespondences(testCase)
            rgbImage = ...
                ProjectionAlignmentObliqueTerrainHarnessTest.texturedRgbImage();
            [scene, truth] = ProjectionAlignmentObliqueTerrainHarness.createScene( ...
                rgbImage, struct(SensorImageSize=[128 128], MeshStride=8));
            options = ProjectionAlignmentOptions.validate(struct( ...
                Detector=struct(Method="sift", MaxFeatures=300, ...
                MaskSupportRadiusPixels=0), Matcher=struct(MaxRatio=0.95), ...
                FilterPipeline=struct(Stages=["geometricOutlier", ...
                "epipolarCoplanarity"], GeometricMethod="similarity", ...
            ...
            working = ProjectionAlignmentWorkingImageRenderer.render( ...
                scene, request, struct(OutputSize=[128 128]));
            matches = ProjectionAlignmentFeatureMatcher.match(working, options);

            first = ProjectionAlignmentMatchFilter.filter( ...
                matches, options, scene);
            second = ProjectionAlignmentMatchFilter.filter( ...
                matches, options, scene);
            metrics = ProjectionAlignmentObliqueTerrainHarness.truthMetrics( ...
                first, truth);
            geometricModel = ...
                first.Diagnostics.FilterPipeline.GeometricModel;
            coplanarity = first.Diagnostics.FilterPipeline.Coplanarity;

            testCase.verifyGreaterThan(matches.Matches.Count, 10);
            testCase.verifyGreaterThanOrEqual(first.Matches.Count, ...
                floor(0.8 * matches.Matches.Count));
            testCase.verifyLessThan(metrics.P95SeparationMeters, 25);
            testCase.verifyEqual(first.Matches.MatchRecordIndices, ...
                second.Matches.MatchRecordIndices);
            testCase.verifyEqual(geometricModel.CoordinateSpace, ...
                "workingPixels");
