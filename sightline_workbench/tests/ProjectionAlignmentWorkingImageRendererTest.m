classdef ProjectionAlignmentWorkingImageRendererTest < matlab.unittest.TestCase
    ...
    methods (Test)
        ...
        function testAnalysisPyramidIsBoundedRadiometricAndReusable(testCase)
            [columnGrid, rowGrid] = meshgrid(1:256, 1:256);
            imageData = 1000 + 2 * rowGrid + 3 * columnGrid + ...
                20 * sin(0.03 * rowGrid);
            scene = ProjectionViewerHarness.createSceneFromImages( ...
                {imageData, imageData + 10, imageData + 20}, ...
                ["large1.tif" "large2.tif" "large3.tif"], ...
                struct(RowStride=8, ColumnStride=8, GSD=0.5, ...
                PlatformStepMeters=0.5));
            request = struct(Options=struct(Scheduling=struct( ...
                Strategy="centerStar", ReferenceLayerIndex=2)));

            working = ProjectionAlignmentWorkingImageRenderer.render( ...
                scene, request, struct(OutputSize=[32 32]));
            source = working.AnalysisSourceDiagnostics;
            layerDiagnostics = [working.PairWorkingImages. ...
                AnalysisSourceDiagnostics];

            testCase.verifyGreaterThanOrEqual(source.CacheHits, 1);
            testCase.verifyEqual(source.SourceReadCount, 3);
            testCase.verifyLessThanOrEqual(source.SourcePixelsRead, ...
                3 * numel(imageData));
            testCase.verifyTrue(all( ...
                [layerDiagnostics.SelectedDownsample] > 1));
            testCase.verifyTrue(all( ...
                [layerDiagnostics.SelectedDownsample] <= ...
                [layerDiagnostics.SourceFootprintPercentile90] ./ ...
                [layerDiagnostics.OversamplingFactor]));
            testCase.verifyTrue(all(string( ...
                {layerDiagnostics.RadiometricOption}) == "nativeSourceBand"));
            validValues = working.LayerImages(1).Image( ...
                working.LayerImages(1).ValidMask);
            testCase.verifyGreaterThan(max(validValues), 1000);
            testCase.verifyEqual(working.LayerImages(1).SourceRows( ...
                working.LayerImages(1).SourceObservationMask), ...
                working.PairWorkingImages(1).LayerImages(1).SourceRows( ...
                working.PairWorkingImages(1).LayerImages(1). ...
                SourceObservationMask));

            tiled = ProjectionAlignmentWorkingImageRenderer.render( ...
                scene, request, struct(OutputSize=[32 32], ...
                AnalysisPyramid=struct( ...
                MaximumMaterializationPixels=4096)));
            testCase.verifyGreaterThan( ...
                tiled.AnalysisSourceDiagnostics.SourceReadCount, 3);
            testCase.verifyEqual(tiled.LayerImages(1).ValidMask, ...
                working.LayerImages(1).ValidMask);
            commonMask = tiled.LayerImages(1).ValidMask & ...
                working.LayerImages(1).ValidMask;
            testCase.verifyEqual(tiled.LayerImages(1).Image(commonMask), ...
                working.LayerImages(1).Image(commonMask), AbsTol=1e-8);
        end
    end
