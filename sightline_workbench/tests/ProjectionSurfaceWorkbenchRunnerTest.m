classdef ProjectionSurfaceWorkbenchRunnerTest < matlab.unittest.TestCase
    ...
    methods (Test)
        function testMultiImageDefaultsAndPassFilteringAreExecutable(testCase)
            ...
            testCase.verifyEqual(configuration.SelectedViewIds, ...
                catalog.ViewIds);
            testCase.verifyEqual(configuration.SelectedPassIds, ...
                catalog.PassIds);
            testCase.verifyEqual(configuration.SelectedPairIds, ...
                catalog.PairIds);
            testCase.verifyEqual(configuration.PairSchedule, "quality");
            testCase.verifyEqual(configuration.ProcessingStage, ...
                "robustMultiView");

            state = ProjectionSurfaceWorkbenchRunnerTest.state( ...
                catalog, configuration, struct( ...
                SelectedPassIds="pass-1", PairSchedule="quality"));
            report = runner.preflight(state);

            testCase.verifyNumElements(report.SelectedPairIds, 3);
            testCase.verifyEqual(report.ScheduledPassIds, "pass-1");
            testCase.verifyNumElements(report.ScheduledViewIds, 3);
            testCase.verifyEqual(report.SelectedPassIds, "pass-1");
            testCase.verifyEqual( ...
                report.ResourceEstimate.MaximumObservationsPerPair, ...
                state.MaximumObservations);
            testCase.verifyTrue(any(contains( ...
                string({report.PairDecisions.Reason}), ...
                "passes are not selected")));
        end

        function testSchedulesHaveDistinctDeterministicSemantics(testCase)
            context = ProjectionSurfaceWorkbenchRunnerTest.context(5);
            runner = ProjectionSurfaceWorkbenchRunner(context, ...
                ProjectionSurfaceWorkbenchRunnerTest.registries());
            catalog = runner.initialCatalog();
            configuration = runner.initialConfiguration(catalog);
            fast = runner.preflight( ...
                ProjectionSurfaceWorkbenchRunnerTest.state( ...
                catalog, configuration, struct(PairSchedule="fast")));
            balanced = runner.preflight( ...
                ProjectionSurfaceWorkbenchRunnerTest.state( ...
                catalog, configuration, struct(PairSchedule="balanced")));
            quality = runner.preflight( ...
                ProjectionSurfaceWorkbenchRunnerTest.state( ...
                catalog, configuration, struct(PairSchedule="quality")));
            plausible = runner.preflight( ...
                ProjectionSurfaceWorkbenchRunnerTest.state( ...
                catalog, configuration, ...
                struct(PairSchedule="allPlausible")));
            explicit = runner.preflight( ...
                ProjectionSurfaceWorkbenchRunnerTest.state( ...
                catalog, configuration, struct(PairSchedule="operator", ...
                SelectedPairIds=catalog.PairIds(1:2))));
            counts = [numel(fast.SelectedPairIds) ...
                numel(balanced.SelectedPairIds) ...
                numel(quality.SelectedPairIds) ...
                numel(plausible.SelectedPairIds) ...
                numel(explicit.SelectedPairIds)];

            testCase.verifyEqual(counts, [1 4 10 10 2]);
        end
        ...
        function testPreflightReportsExactBoundedProposalAndFallback(testCase)
            ...
            testCase.verifyEqual( ...
                report.ResourceEstimate.RequestedAssociationRecordCount, 24);
            testCase.verifyEqual( ...
                report.ResourceEstimate.MaximumScheduledAssociationRecords, 24);
            testCase.verifyFalse( ...
                report.ResourceEstimate.AssociationBudgetApplied);
        ...
        function testTotalAssociationBudgetIsDistributedDeterministically(testCase)
            [runner, catalog, configuration] = ...
                ProjectionSurfaceWorkbenchRunnerTest.fixture();
            state = ProjectionSurfaceWorkbenchRunnerTest.state( ...
                catalog, configuration, struct(DenseMethod="external", ...
                PairSchedule="quality", MaximumObservations=8, ...
                MaximumAssociationRecords=10));

            report = runner.preflight(state);
            outcome = runner.run(state);

            testCase.verifyEqual( ...
                report.ResourceEstimate.AssociationRecordsPerPair, [4 3 3]);
            testCase.verifyTrue( ...
                report.ResourceEstimate.AssociationBudgetApplied);
            testCase.verifyNotEmpty( ...
                report.ResourceEstimate.AssociationBudgetAdvice);
            testCase.verifyEqual(sum([outcome.PairRuns.RecordCount]), 10);
            testCase.verifyEqual( ...
                outcome.Association.Diagnostics.InputPairRecordCount, 10);
        end

        function testPairFailureRetainsExactExportableEvidence(testCase)
            context = ProjectionSurfaceWorkbenchRunnerTest.context();
            failedPairId = context.PairEntries(2).PairId;
            matcherRegistry = ProjectionDenseMatcherRegistry({ ...
                ProjectionDenseSgmMatcher(), ...
                ProjectionDenseTemplateMatcher(), ...
                ProjectionFailingDenseMatcherFixture(failedPairId)});
            fusionRegistry = ProjectionSurfaceFusionRegistry({ ...
                ProjectionRobustMultiRayFusion()});
            runner = ProjectionSurfaceWorkbenchRunner(context, struct( ...
            ...
            state = ProjectionSurfaceWorkbenchRunnerTest.state( ...
                catalog, configuration, struct(DenseMethod="external", ...
                PairSchedule="quality", MaximumObservations=4));
            path = string(tempname) + ".mat";
            testCase.addTeardown(@() delete(path));

            surfaceWorkbenchRun = runner.run(state);
            save(path, "surfaceWorkbenchRun", "-v7");
            loaded = load(path, "surfaceWorkbenchRun");
            failed = loaded.surfaceWorkbenchRun.PairRuns(2);
            completed = loaded.surfaceWorkbenchRun.PairRuns(1);

            testCase.verifyEqual(loaded.surfaceWorkbenchRun.Status, "partial");
            testCase.verifyEqual(failed.PairId, failedPairId);
            testCase.verifyEqual(failed.MatcherAlgorithmId, ...
                "test.failure-capture");
            testCase.verifyEqual(failed.Identifier, ...
                "ProjectionDenseMatcher:algorithmFailure");
            testCase.verifyEqual(failed.CauseIdentifier, ...
                "ProjectionFailingDenseMatcherFixture:injectedFailure");
            testCase.verifyEqual(failed.Failure.ExceptionIdentifier, ...
                failed.CauseIdentifier);
            testCase.verifyEqual(failed.Failure.ExceptionMessage, ...
                failed.CauseMessage);
            testCase.verifyTrue(contains(failed.Message, "Injected pair failure"));
            testCase.verifyEqual(failed.Stage, "matching");
            testCase.verifyEqual(failed.Options.Count, 4);
            testCase.verifyGreaterThanOrEqual(failed.ElapsedSeconds, 0);
            testCase.verifyEqual(completed.Stage, "completed");
            testCase.verifyTrue( ...
                completed.Evidence.CompleteIntermediateEvidenceRetained);
            testCase.verifyEqual( ...
                loaded.surfaceWorkbenchRun.LastCompletedPairId, ...
                context.PairEntries(3).PairId);
            testCase.verifyFalse(ProjectionSurfaceWorkbenchFixture. ...
                hasRuntimeHandle(loaded.surfaceWorkbenchRun));
        end

        function testAssociationCancellationRetainsPairsAndStageDiagnostics(testCase)
            [runner, catalog, configuration] = ...
                ProjectionSurfaceWorkbenchRunnerTest.fixture();
            state = ProjectionSurfaceWorkbenchRunnerTest.state( ...
                catalog, configuration, struct(DenseMethod="external", ...
            ...
            recorder.CancelStage = "association";

            outcome = runner.run(state, struct( ...
                ProgressFcn=@(event) recorder.recordRunner(event), ...
                CancellationFcn=@() recorder.cancel()));

            testCase.verifyEqual(outcome.Status, "cancelled");
            testCase.verifyEqual(outcome.ProcessingStage, "association");
            testCase.verifyNumElements(outcome.PairRuns, 3);
            testCase.verifyTrue(all(string({outcome.PairRuns.Status}) == ...
                "succeeded"));
            testCase.verifyEqual(outcome.StageDiagnostics.ParentStage, ...
                "association");
            testCase.verifyEqual(outcome.StageDiagnostics.Stage, "normalize");
